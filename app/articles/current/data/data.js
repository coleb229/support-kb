export const data = {
  ...
  pushTasks: {
    steps: [
      'Login to https://paxus.paxstore.us/portal',
      'Click on the little guy in the top right corner to drop down the marketplace list',
      'Click on the "Administration Center" Icon',
      'From the left navigation menu, click on "Terminal Management"',
      'Click on the magnifying glass icon to search for the merchant, or search by the serial number of the device',
      'Click on the terminal name to open the terminal profile',
      'From this page you can view terminal stats and connection status as well as installed apps',
      'Click on the tab named "Push Task"',
      'From this page you can choose to push a previous task by clicking the dropdown carrot to the right of "Operation History" OR you can create a brand new task by clicking "+ Push App"',
      'For these steps, we will be creating a new task, so click the + Push Task button',
      'This opens a new menu, where you can use the search bar in the top-right corner to search for the app needed',
      'Select the version to be pushed and then tab the app name to add it to the task',
      'Click "OK" to start the template view, where you can set the parameters, or settings, for the processing app',
      'From here you will want to first adjust the "MISC" settings, so click on that tab',
      'Make sure the ECR-Terminal Integration Mode is set to "External POS" for merchants using the POS app, otherwise set this to "Internal POS" if they are using QST or Qlite',
      'Next, navigate to the TSYS tab and fill in the information from the VAR sheet and double check it matches perfectly and then make sure P2PE Mode is disabled',
      'Finally, navigate to the "COMMUNICATION" tab and set the "Protocol Type" to "HTTP GET" for external pos users, or "TCP/IP" if they are set to internal',
      'Once that is all done, click on "NEXT" in the bottom-right corner of the form to proceed to the final page before pushing the task',
      'On this page, you dont usually need to change any of the thre toggle settings above the timezone, but make sure the timezone is set correctly to match the merchants location',
      'Once thats set, you can click "ACTIVATE" to start the push',
      'The terminal will beep 3 times and then the install will be complete',
    ],
    useCases: ['Updating tsys/TransIT'],
  },
  gatewaySetup: {
    steps: [
      'Pull up the account in the Partner Portal and navigate to the "Stations" tab',
      'Click on the green + button to add a new station',
      'Set the app type based on the software being used on this station, the terminal name, and display name, then hit save',
      'Now that the station is created, click on the lock icon to the right of it to open the payment gateway setup.  These settings will pair the station to its respective card reader',
      'The settings you can see on this screen are determined by the "Payment Gateway" setting in the top-right.  I am using PAX Direct for the demonstration because it is most common',
      'The main settings to worry about for simply pairing will be the "App Type", "Payment Gateway", "Terminal IP (or SN if using PAX(QST/LITE/APIO))", and "Program/Label/Value" settings',
      'Click Save once everything is set to your needs and the station will be paired to the reader',
    ],
    useCases: [
      'Setting up a payment terminal',
      'Adjusting terminal integration settings',
    ],
    settingsKey: [
      'App Type*: This is the software being used on the station.  This will determine the app this station will be using',
      'Payment Gateway*: This is the payment gateway being used for the station.  This will determine how the POS can interact with the specific payment gateway',
      'Merchant ID: This is the merchant ID for the payment gateway.  This is used to identify the merchant to the payment gateway',
      'Serial Number*: This is the serial number of the terminal.  This is used to identify the terminal to the payment gateway',
      'Terminal IP*: This is the IP address of the terminal.  This is used to identify the network location of the terminal to the payment gateway',
      'Terminal Model: This is the model of the terminal.  This is used to label the terminal model.',
      'Enable External Tip Display: This is used to enable the external tip display on the terminal.  This will flip the tip prompt from the reader screen to an external screen',
      'Hide cash/card prompt: This will hide the cash/card prompt on the terminal.',
      'Enable Reader Tip: I think this makes tsys display its internal tip prompt on pax direct',
      'Enable Debit Pin: This will allow a prompt to display that allows the choice between credit or debit, if the customer is using a debit card',
      'Batch Status (One Shot): Not sure what this does',
      'Auto Batching: This will automatically batch the terminal at the end of the day, based on the time they set. (**Does not work with PAX Direct/Magensa**)',
      'Pre Auth Tab Amount: This determines the amount run on each card when tokenizing card-data to close out remotely from the POS after the customer signs their receipt tab (Frequently used for bars)',
      'Enable External Printer: This will cause handhelds/E700/800s to print to external receipt printers. (This will not stop the internal printer from printing, you need to adjust tsys settings for that)',
      'Request Time Out (Seconds): This is the amount of time the POS will wait for a response from the terminal before timing out during payment processing',
      'Manual Card Integrated: Not certain what this does, something to do with manual payments',
      'Disable Token: This will restrict the tokenization, or storage, of card data in the reader',
      'Program: This will determine how surcharge fees are handled.  Do not touch this without direct consent from the agent of the account.',
      'Label: This will label the surcharge fee.',
      'Value: This will set the amount of the surcharge fee.',
      'Value Type: This will determine if the surcharge fee is a percentage or a flat fee.',
      'Tip Prompt: This will determine if the tip prompt is displayed on the terminal after card payments.',
      'Disable Surcharge On Tip Prompt: This will disable the surcharge fee if the customer adds a tip to the payment through tip prompt.',
      'Enable Tip Adjustment: This will allow the customer to add their tip to receipts, and the merchant to add and adjust it from the tip section on closed orders.',
      'Enable Surcharge: This will enable the surcharge fee (on tip adjustment?)',
    ],
  },
  paymentPrograms: {
    steps: ['', '', '', '', '', '', '', '', ''],
